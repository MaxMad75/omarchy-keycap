...
Item {
  ...
  // Resolve the bindings once at load, so the first open already has them.
  ...
  // Keybinding hints. omarchy-menu-shortcuts resolves every Hyprland binding
  // to the thing it opens and answers with four maps: byApp keyed on desktop
  // id, byCmd on the exact command a row runs, byRoute on the submenu a
  // binding opens, byName on the binding's own description. A row is looked up
  // in that order, so the precise answer always beats the name match.
  // The manifest arrives with __sourceDir stamped in by the plugin registry,
  // which is how the bundled resolver is found without touching PATH.
  readonly property string pluginDir: (root.manifest && root.manifest.__sourceDir)
    ? String(root.manifest.__sourceDir) : ""
  property var shortcuts: ({})
  property int shortcutsSerial: 0
  property var shortcutByItem: ({})
  property string shortcutIndexStamp: ""
  ...
  // Routes resolve through the whole item list, so the answer is cached and
  // only recomputed when the bindings or the item set actually move.
  function ensureShortcutIndex() {
    var stamp = root.shortcutsSerial + ":" + root.itemOrder.length
    if (stamp === root.shortcutIndexStamp) return
    root.shortcutIndexStamp = stamp

    var index = ({})
    var byRoute = (root.shortcuts && root.shortcuts.byRoute) || ({})
    for (var route in byRoute) {
      var id = MenuModel.resolveRoute(root.items, root.itemOrder, route)
      if (id && !index[id]) index[id] = byRoute[route]
    }
    root.shortcutByItem = index
  }

  function shortcutFor(entry) {
    if (!entry || !root.shortcuts) return ""

    if (entry.kind === "app") {
      var byApp = root.shortcuts.byApp || ({})
      var appHit = byApp[String(entry.appId || "")]
      if (appHit) return appHit
    }

    if (entry.action) {
      var byCmd = root.shortcuts.byCmd || ({})
      var cmdHit = byCmd[String(entry.action).trim()]
      if (cmdHit) return cmdHit
    }

    var routeHit = root.shortcutByItem[entry.id]
    if (routeHit) return routeHit

    var byName = root.shortcuts.byName || ({})
    return byName[String(entry.label || "").toLowerCase()] || ""
  }
  ...
  function rebuildDisplay() {
    ...
    if (query) {
      ...
      for (var i = 0; i < root.itemOrder.length; i++) {
        ...
        row.shortcut = root.shortcutFor(entry)
      ...
      for (var j = 0; j < root.itemOrder.length; j++) {
        ...
        var childRow = root.displayRow(child, child.description, child.order)
        childRow.shortcut = root.shortcutFor(child)
        rows.push(childRow)
      }
  ...
  // Answers from a cache keyed on `hyprctl binds` plus the application dirs, so
  // repeat opens cost milliseconds and a rebind is picked up on the next one.
  Process {
    id: shortcutProc
    property string collected: ""
        ...
        Item {
          ...
          ListView {
            ...
            delegate: BorderSurface {
              ...
              // The hint sits between the label and the chevron, so the label
              // elides against it rather than running underneath.
              Text {
                id: shortcutText
                visible: row.shortcut.length > 0
                text: row.shortcut
                color: row.hasCursor ? root.selectedText : root.foreground
                opacity: row.hasCursor ? 0.7 : 0.4
                font.family: root.fontFamily
